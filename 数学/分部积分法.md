...
## $$ \begin{align*} \arctan x = I_1 &= \int \frac{\mathrm{d}x}{1+x^2} = \int \frac{1}{1+x^2} \mathrm{d}x \\ &= \frac{x}{1+x^2} + \int x \cdot \frac{2x}{(1+x^2)^2} \mathrm{d}x \\ &= \frac{x}{1+x^2} + 2\int \frac{x^2+1-1}{(1+x^2)^2} \mathrm{d}x \\ &= \frac{x}{1+x^2} + 2\arctan x - 2\int \frac{\mathrm{d}x}{(1+x^2)^2} \\ I_1 &= \frac{x}{1+x^2} + 2I_1 - 2I_2 \end{align*} $$ --- ### 补充整理（对应板书最终求解） 由 $I_1 = \frac{x}{1+x^2} + 2I_1 - 2I_2$ 移项整理： $$ \begin{align*} 2I_2 &= \frac{x}{1+x^2} + 2I_1 - I_1 \\ 2I_2 &= \frac{x}{1+x^2} + I_1 \\ I_2 &= \frac{x}{2(1+x^2)} + \frac{1}{2}\arctan x + C \end{align*} $$ --- ### 关键步骤说明（对应板书标注） 1. 第一步分部积分：令 $u=\frac{1}{1+x^2}$，$\mathrm{d}v=\mathrm{d}x$，得 $\mathrm{d}u=\frac{-2x}{(1+x^2)^2}\mathrm{d}x$，$v=x$，代入公式 $\int u\mathrm{d}v=uv-\int v\mathrm{d}u$ 得到第二步； 2. 分子拆分：将 $x^2$ 拆为 $(x^2+1)-1$，凑出可积分的项； 3. 递推关系：用 $I_1$ 表示 $\arctan x$，$I_2$ 表示目标积分，得到递推式后移项求解。 --- 要不要我把这个推导和之前的递推公式合并成一份**完整的递推型笔记**，方便你直接导入Obsidian？
### $$$$