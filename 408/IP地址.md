...
1. H1 判断是否在同一网络(*先判断目的地址是否在同一网络，不在就交给默认网关”，是对结果的描述；而“查路由表”是实现这个判断的机制。两者说的是同一件事的两个层面)，检查源地址和目的地址的网络号(B类16bit*)主机怎么知道“是不是同一网络”？——就是查自己的路由表
2. H1把数据交给默认网关，通过ARP协议，获取网关的MAC地址
![](assets/IP地址/file-20260926174517156.png)
>**第 1 步：主机要访问外网（比如 8.8.8.8）**  
查路由表 → 命中默认路由 → 下一跳 = `192.168.1.255`，且该地址在本机直连网段内。

**第 2 步：主机对 `192.168.1.255` 发 ARP 请求**

> “Who has 192.168.1.255? Tell 192.168.1.10”

**第 3 步：没人应答。**  
因为 `192.168.1.255` 是子网的广播地址，**没有任何一台设备（包括路由器）会把它绑在自己的接口上**。路由器接口上只有单播地址（如 `192.168.1.1`），广播地址只是个“逻辑名”，不参与 ARP。

**结果：ARP 表项停留在 `(incomplete)`，帧封装不出来，包根本出不了网卡。**  
表现就是 `ping 8.8.8.8` 返回 **Destination Host Unreachable**（注意：是本机自己报的不可达，不是超时——说明包连第一跳都没出去）。
