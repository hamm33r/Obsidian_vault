---
tags:
  - 计算机网络
---
# 各种协议之间的服务关系
![](assets/IPv4分组/file-20260921164923463.png)

# IP数据报（分组）的格式
![](assets/IPv4分组/file-20260921165355179.png)
>IP首部总共20个字节，被分成了5行，每行32bit（4字节）来展示。每一行都是紧接着上一行的，是拼接关系，不是覆盖关系
>例题![](assets/IPv4分组/file-20260921165551326.png)
>IP数据报的理论长度实际上受到数据链路层最短帧长和最长帧长的限制
## IP数据报的分片问题
![](assets/IPv4分组/file-20260921165756032.png)
>最大传送单元（MTU）
>以太网的MTU=1500(这是规定)

例子
![](assets/IPv4分组/file-20260921170046392.png)
>4000>1500链路2需要分片![](assets/IPv4分组/file-20260921171111246.png)
**原始数据报的总长度并没有单独保存在某个字段里**，它是重组时算出来的总长度字段表示的**是这个分片自己的总长度**
### 怎么组装分片
![](assets/IPv4分组/file-20260921172511503.png)
>受到的分片如果标识一样，源地址也一样，那么就属于同一个IP数据报
>如果DF=1，然后这个链路的MTU不允许IP数据报这么大，路由器会把这个不允许分片又发不出去的数据报丢弃，给数据报的源主机发送ICMP报文[终点不可达](ICMP协议.md#终点不可达)，用来通知源主机发生了某种异常情况
>目的主机根据标识标志还有片偏移进行组装![](assets/IPv4分组/file-20260921172741961.png)

## IP数据报的生存时间TTL
![](assets/IPv4分组/file-20260921182203995.png)
![](assets/IPv4分组/file-20260921181937085.png)
![](assets/IPv4分组/file-20260921182023859.png)

# 总结
第一行
![](assets/IPv4分组/file-20260921165355179.png)

第二行![](assets/IPv4分组/file-20260921172511503.png)

第三行![](assets/IPv4分组/file-20260921182203995.png)

第4,5行![](assets/IPv4分组/file-20260921182406106.png)
>IP地址

![](assets/IPv4分组/file-20260921182602060.png)
>418首总偏![](assets/IPv4分组/file-20260921182652547.png)
>首部长度是以4B为单位，总长度是1B为单位，片偏移是8B为单位