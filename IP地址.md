...
1. H1把数据交给默认网关，通过ARP协议，获取网关的MAC地址
2. H1将数据封装成帧，并发送给默认网关
...
||目的 MAC|`MAC_B2`（网关）|
3. 学校路由器从 **B2 接口**收到帧，拆掉 MAC 帧头，提取 IP 数据报，查转发表：发现**网络号**能够与转发表第二个表项匹配上，于是从B0转发出去，需要修改MAC帧的目的地址
...
4. 公司路由器从 C0 接口收到帧，拆掉 MAC 帧头，查转发表：
...
5. H7 收到 MAC 帧，检查目的 MAC 是自己，拆掉 MAC 帧头，提取 IP 数据报，检查目的 IP 是自己（`200.1.1.2`），接收数据。