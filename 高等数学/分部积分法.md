...
> 按照反对幂指三这样的积分难度递减顺序，积分难度高的当做u来求导，排前面的留下来，排后面的放在d后面当v
...
![](assets/分部积分法/file-20260908210757602.png)

# 分部积分法：建立欲求积分的方程（以 $\int e^t \sin t \, dt$ 为例） 
---
 ## 一、（循环型）题目 求不定积分： $$ \int e^t \sin t \, dt $$ 
---
...
---
# e的x次幂×三角函数积分通用公式($\int e^{ax}\sin bx dx$ / $\int e^{ax}\cos bx dx$) 
%%e的x次幂乘sinx/cosx%%
## 一、通用公式（可直接套用）  
...
## $$\begin{align*} \arctan x = I_1 &= \int \frac{\mathrm{d}x}{1+x^2} = \int \frac{1}{1+x^2} \mathrm{d}x \\ &= \frac{x}{1+x^2} + \int x \cdot \frac{2x}{(1+x^2)^2} \mathrm{d}x \\ &= \frac{x}{1+x^2} + 2\int \frac{x^2+1-1}{(1+x^2)^2} \mathrm{d}x \\ &= \frac{x}{1+x^2} + 2\arctan x - 2\int \frac{\mathrm{d}x}{(1+x^2)^2} \\ I_1 &= \frac{x}{1+x^2} + 2I_1 - 2I_2 \\ \Rightarrow I_2 &= \frac{x}{2(1+x^2)} + \frac{1}{2}\arctan x + C \end{align*}$$
### $$$$