...
![](assets/DMA方式/file-20260608215719137.png)
...
- **DMA请求（HRQ）**：是DMA控制器向**总线仲裁器**发出的“我要用总线”的申请。CPU在当前总线周期结束后，会自动释放总线控制权（发出HLDA信号），这个过程对CPU程序是透明的，CPU不会被打断。
...
# DMA 传送方式
![](assets/DMA方式/file-20260608214907561.png)
>解决DMA和CPU的访存冲突

# DMA方式的特点
![](assets/DMA方式/file-20260608215053534.png)
>只有像这个图里的[三总线结构](408/总线的分类.md#三总线结构)才需要考虑[DMA 传送方式](#DMA%20传送方式)里的访存冲突问题，因为这个时候有一个单独的DMA总线，不归CPU管了
>而[DMA 传送方式](#DMA%20传送方式)里的单总线结构是没有这个问题，DMA什么时候访存都是CPU说的算的
